...
Kernel::Kernel()
{
    ...
    spindleon = false;
    cachewait = false;
    Play_SpindleFan = false;
    b_AutoClean = false;
    b_3DProbeMode = false;
    Play_SpindleFan_value = 30;

    instance = this; // setup the Singleton instance of the kernel    
    power_24V_time = 0xffffffff;
...
*/
	...
	// get blowing_mode and bedclean_mode and static electricity removal mode
	n= snprintf(buf, sizeof(buf), ",%d,%d,%d,%d", int(this->get_blowing_mode()),int(this->get_bedclean_mode()),0,int(this->get_elecremove_mode()));
    if(n > sizeof(buf)) n= sizeof(buf);
    str.append(buf, n);
    ...
    {
    	n = snprintf(buf, sizeof(buf), "|C:%d,%d,%d,%d", CARVERA_AIR,THEKERNEL->factory_set->FuncSetting,THEROBOT->inch_mode,THEROBOT->absolute_mode);
    }
    else if(MAKERA_Z1 == THEKERNEL->factory_set->MachineModel)
    {
    	n = snprintf(buf, sizeof(buf), "|C:%d,%d,%d,%d", MAKERA_Z1,THEKERNEL->factory_set->FuncSetting,THEROBOT->inch_mode,THEROBOT->absolute_mode);
    }
    else if(MAKERA_Z1Pro == THEKERNEL->factory_set->MachineModel)
    {
    	n = snprintf(buf, sizeof(buf), "|C:%d,%d,%d,%d", MAKERA_Z1Pro,THEKERNEL->factory_set->FuncSetting,THEROBOT->inch_mode,THEROBOT->absolute_mode);
    }
    else
    {
...
std::string Kernel::get_diagnose_string()
{
    ...
    if( (CARVERA_AIR == THEKERNEL->factory_set->MachineModel) || ( MAKERA_Z1 == THEKERNEL->factory_set->MachineModel ) || ( MAKERA_Z1Pro == THEKERNEL->factory_set->MachineModel ))
	{	
    	bool ok2 = false;
    	bool ok3 = false;
    	int pad2value = 0;
	    ...
	    {
	    	if(pad2.value>19 && pad2.value<=21)				//20Hz
	    	{
	    		pad2value = 1;	
	    	}
	    	else if(pad2.value>=9 && pad2.value<=11)		//10Hz
	    	{
	    		pad2value = 2;	
	    	}
	    	else if(pad2.value>=4 && pad2.value<=6)			//5Hz
	    	{
	    		pad2value = 3;	
	    	}
	    	else if(pad2.value>2 && pad2.value<3)			//2.5Hz
	    	{
	    		pad2value = 4;	
	    	}
	    	else if(pad2.value>1.5 && pad2.value<=2.5)		//2Hz
	    	{
	    		pad2value = 5;	
	    	}
	    	else
        ...
        n = snprintf(buf, sizeof(buf), ",%d,%d,%d,%d", (int)pad.state, (int)pad2value, (int)pad3.state, (int)pad3.value);
        if(n > sizeof(buf)) n = sizeof(buf);
        str.append(buf, n);
...
{
	unsigned int size = 4+4;	//0x5A 0xA5 DATA(4byte) CRC(2byte)
	char i2c_buffer[size];
	...
	if( Check_Factory_Data((unsigned char*)i2c_buffer, 4+2 ) )
	{
    	this->probe_addr = i2c_buffer[2]<< 24 + i2c_buffer[3]<<16 + i2c_buffer[4]<<8 + i2c_buffer[5];
    }
    else
    {
    	this->probe_addr = 0;
...
void Kernel::write_ProbeAddr_Data_data(uint32_t Addr)
{
	unsigned int size = 4;
	unsigned int datalen = size + 4;
	char Data_buffer[datalen];
	...
	unsigned int bytenum =0;
	unsigned char * writeptr = 0;
	unsigned int u8Pagebegin=EEPROM_PROBEADDR_PAGE;
	
	Data_buffer[0] = 0x5A;
	Data_buffer[1] = 0xA5;
	Data_buffer[2] = (Addr>>24) & 0xff;
	Data_buffer[3] = (Addr>>16) & 0xff;
	Data_buffer[4] = (Addr>>8) & 0xff;
	Data_buffer[5] = Addr & 0xff;
	...
	if (result != 0) {
		this->streams->printf("ALARM: Probe Address data write error:%d\n",pagenum);
	} 
}
...
uint8_t Kernel::process_factory_command(dfu_frame_t *frame, dfu_state_t *dfu_state, uint32_t *current_frame, uint32_t *total_frames, bool *bneedwrite) {
    switch (frame->type) {
        case PTYPE_FACTORY_START:
            ...
            THEKERNEL->serial->send_cfg_frame(PTYPE_FACTORY_VIEW, response_data, 6);
            break;            
        case PTYPE_FACTORY_VIEW:
        	if (*dfu_state == DFU_STATE_START && frame->length >= 6) {
                ...
                *dfu_state = DFU_STATE_FILEVIEW;  
                ...
                THEKERNEL->serial->send_cfg_frame(PTYPE_FACTORY_DATA, request_data, 4);
            }
            break;            
                ...
                *dfu_state = DFU_STATE_TRANSFERRING;
                ...
                if (frame->length >= 5) {
                    ...
                    if (frame_num == *current_frame + 1) {
						...
						string line(reinterpret_cast<char*>(file_data), data_len);
						Factroy_ProcessLine(line, bneedwrite);
						*current_frame = frame_num;
                         // ???????????
                        if (*current_frame >= *total_frames) {
                            // ????????????PTYPE_FILE_END
                            THEKERNEL->serial->send_cfg_frame(PTYPE_FACTORY_END, NULL, 0);
                ...
                THEKERNEL->serial->send_cfg_frame(PTYPE_FACTORY_CAN, &error_code, 1);
                *dfu_state = DFU_STATE_IDLE;
            }
            break;
            
        case PTYPE_FACTORY_END:            
        case PTYPE_FACTORY_CAN:
            THEKERNEL->serial->send_cfg_frame(PTYPE_FACTORY_END, NULL, 0);
            *dfu_state = DFU_STATE_IDLE;
            return 1;
            break;
...
bool Kernel::Factroy_ProcessLine(string& line, bool * bneedwrite)
{
...
void Kernel::read_Factroy_SD()
{
    ...
    THEKERNEL->serial->send_cfg_frame(PTYPE_FACTORY_START, NULL, 0);
    ...
    while (wait_count < MAX_WAIT_COUNT && !start_ack_received) {
        ...
        if (result == 0) {
            process_factory_command(&frame, &dfu_state, &current_frame, &total_frames, &bneedwrite);
            
            if (frame.type == PTYPE_FACTORY_START) {
                start_ack_received = 1;
                dfu_state = DFU_STATE_START;
                continue;
            }
			else if (frame.type == PTYPE_FACTORY_CAN){
				return;
			}
        }
    ...
    if (!start_ack_received) {
        THEKERNEL->serial->send_cfg_frame(PTYPE_FACTORY_CAN, NULL, 0);
        return;
    }
    ...
    while (1) {
        int result = THEKERNEL->serial->receive_cfg_frame(&frame, 100);            
        if (result == 0) {
            if ((frame.type&0xf0) == 0xe0) {
				wait_count = 0;
				if(process_factory_command(&frame, &dfu_state, &current_frame, &total_frames, &bneedwrite)){
					break;
				}
			}        
        } else {
