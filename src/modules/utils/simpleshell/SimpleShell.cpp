    ...
    if (gcode->has_m) {
        if (gcode->m == 20) { // list sd card
        	...
        	if (gcode->subcode == 0) {
				...
				THEKERNEL->set_blowing_mode(true);
				if (gcode->has_letter('S')) {
					uint16_t value = gcode->get_int('S');
					if( value>=0 && value <=100)
                    	THEKERNEL->set_PlaySpindleFanValue(value);
                }
				// turn on auto blowing mode
				PacketMessage(PTYPE_NORMAL_INFO, "turning auto blowing mode on\r\n", 0, gcode->stream);
			}
			else if (gcode->subcode == 2) {
				
				THEKERNEL->set_bedclean_mode(true);
				// turn on auto bed clean mode
			...
			if (gcode->subcode == 0) {
				...
				THEKERNEL->set_blowing_mode(false);
				// turn off auto blowing mode
				PacketMessage(PTYPE_NORMAL_INFO, "turning auto blowing mode off\r\n", 0, gcode->stream);
			}
			else if (gcode->subcode == 2) {
				
				THEKERNEL->set_bedclean_mode(false);
				// turn off auto bed clean mode
...
void SimpleShell::on_console_line_received( void *argument )
{
    ...
    if(possible_command[0] == '$' && possible_command.size() >= 2) {
        ...
        } else if (cmd == "laserabort") {	//lsf add 2026.03.20
            // these are handled by Laser module
...
void SimpleShell::diagnose_command( string parameters, StreamOutput *stream)
{
    ...
    if (ok) {
	    ...
	    struct pad_temperature temp1,temp2;
	    PublicData::get_value( temperature_control_checksum, current_temperature_checksum, spindle_temperature_checksum, &temp1 );
	    // get power temperature
    	PublicData::get_value( temperature_control_checksum, current_temperature_checksum, power_temperature_checksum, &temp2 );
        n = snprintf(buf, sizeof(buf), "S:%d,%d,0,0,%d,%d", (int)ss.state, (int)ss.target_rpm,(int)temp1.current_temperature,(int)temp2.current_temperature);
...
{	
	...
	{
		case CARVERA:
			stream->printf("model = %s, %d, %d, %u, %s\n", "C1", THEKERNEL->factory_set->MachineModel, THEKERNEL->factory_set->FuncSetting, THEKERNEL->probe_addr, str.c_str());
			break;
		case CARVERA_AIR:
			stream->printf("model = %s, %d, %d, %u, %s\n", "CA1", THEKERNEL->factory_set->MachineModel, THEKERNEL->factory_set->FuncSetting, THEKERNEL->probe_addr, str.c_str());
			break;
		case MAKERA_Z1:
			stream->printf("model = %s, %d, %d, %u, %s\n", "Z1", THEKERNEL->factory_set->MachineModel, THEKERNEL->factory_set->FuncSetting, THEKERNEL->probe_addr, str.c_str());
			break;
		case MAKERA_Z1Pro:
			stream->printf("model = %s, %d, %d, %u, %s\n", "Z1", THEKERNEL->factory_set->MachineModel, THEKERNEL->factory_set->FuncSetting, THEKERNEL->probe_addr, str.c_str());
			break;
...
void SimpleShell::fget_command( string parameters, StreamOutput *stream)
{
	...
	switch (THEKERNEL->factory_set->MachineModel)
	{
		...
		case MAKERA_Z1:
			...
			break;
		case MAKERA_Z1Pro:
			stream->printf("machine = %s, %d, %d, %u, %s\n", "Z1Pro", THEKERNEL->factory_set->MachineModel, THEKERNEL->factory_set->FuncSetting, THEKERNEL->probe_addr, str.c_str());
			break;
		default:
			stream->printf("machine = %s, %d, %d, %u, %s\n", "C1", THEKERNEL->factory_set->MachineModel, THEKERNEL->factory_set->FuncSetting, THEKERNEL->probe_addr, str.c_str());
			break;
...
void SimpleShell::PacketMessage(char cmd, const char* s, int size, StreamOutput *stream)
{
	...
	stream->puts((char *)temp_buf, len+6);
	if (temp_buf != NULL){
        free(temp_buf);
    }
}
