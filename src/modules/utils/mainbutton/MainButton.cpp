...
#define ledBrightness_checksum						CHECKSUM("ledBrightness")
...
void MainButton::on_module_loaded()
{
	...
	else if((MAKERA_Z1 == THEKERNEL->factory_set->MachineModel) || (MAKERA_Z1Pro == THEKERNEL->factory_set->MachineModel))
    {
    	this->main_button.from_string( THEKERNEL->config->value( main_button_pin_checksum )->by_default("1.8!^")->as_string())->as_input();
	    ...
	    this->main_button_LED_G.from_string( THEKERNEL->config->value( main_button_LED_G_pin_checksum )->by_default("0.22")->as_string())->as_output();
    ...
    if((MAKERA_Z1 == THEKERNEL->factory_set->MachineModel) || (MAKERA_Z1Pro == THEKERNEL->factory_set->MachineModel))
    {
	    this->PS12.from_string( THEKERNEL->config->value( ps12_pin_checksum )->by_default("2.12")->as_string())->as_output();
	    this->PS24.from_string( THEKERNEL->config->value( ps24_pin_checksum )->by_default("3.26")->as_string())->as_output();
    }
    ...
    this->light_brightness = THEKERNEL->config->value(light_checksum, ledBrightness_checksum) ->by_default(15)->as_number();
	...
	else if((CARVERA_AIR == THEKERNEL->factory_set->MachineModel) || (MAKERA_Z1 == THEKERNEL->factory_set->MachineModel) || (MAKERA_Z1Pro == THEKERNEL->factory_set->MachineModel))
    {
...
{
	this->PS24.set(state);
	if(state)	//lsf modify 2026.03.20
	{
		THEKERNEL->power_24V_time = us_ticker_read();
	}
	else
	{
		THEKERNEL->power_24V_time = 0xffffffff;
	}
}
...
uint32_t MainButton::led_tick(uint32_t dummy)
{
	...
	switch (state) {
		case HOLD:
			this->hold_toggle ++;
			if((this->hold_toggle % 4 == 0) || (this->hold_toggle % 4 == 3) )
				this->set_led_colors(this->hold_toggle % 4  < 2 ? this->light_brightness : 0, 0, 0);
			break;
		case SUSPEND:
			this->hold_toggle ++;
			if((this->hold_toggle % 4 == 0) || (this->hold_toggle % 4 == 3) )
				this->set_led_colors(0, 0, this->hold_toggle % 4 < 2 ? this->light_brightness : 0);
			break;
		case WAIT:
			this->hold_toggle ++;
			if((this->hold_toggle % 4 == 0) || (this->hold_toggle % 4 == 3) )
				this->set_led_colors(this->hold_toggle % 4 <2 ? this->light_brightness : 0, this->hold_toggle % 4  < 2 ? this->light_brightness : 0, 0);
			break;
		case TOOL:
			this->hold_toggle ++;
    		...
    		switch(tool.target_tool)
    		{
    			case 1:
    				if(this->hold_toggle % 5 == 0)
						this->set_led_num(this->light_brightness,0,this->light_brightness,0,0,0,1);
    			...
    			case 2:
    				if(this->hold_toggle % 5 == 0)
						this->set_led_num(this->light_brightness,0,this->light_brightness,0,0,0,2);
					if(this->hold_toggle % 5 == 1)
						this->set_led_colors(0, 0, 0);
					if(this->hold_toggle % 5 == 2)
						this->set_led_colors(0, 0, 0);
    			...
    			case 3:
    				if(this->hold_toggle % 5 == 0)
						this->set_led_num(this->light_brightness,0,this->light_brightness,0,0,0,3);
					if(this->hold_toggle % 5 == 1)
						this->set_led_colors(0, 0, 0);
					if(this->hold_toggle % 5 == 2)
						this->set_led_colors(0, 0, 0);
    			...
    			default:
    				if(this->hold_toggle % 5 == 0)
						this->set_led_colors(this->light_brightness, 0, this->light_brightness);
					if(this->hold_toggle % 5 == 1)
						this->set_led_colors(0, 0, 0);
					if(this->hold_toggle % 5 == 2)
						this->set_led_colors(this->light_brightness, 0, this->light_brightness);
	...
	if (state != old_state) 
	{
		...
		switch (state) {
			case IDLE:
				this->set_led_colors(0, 0, this->light_brightness);
				break;
			case RUN:
				this->set_led_colors(this->light_brightness, 0, 0);
				break;
			case HOME:
				this->set_led_colors(this->light_brightness, this->light_brightness, 0);
				break;
			case ALARM:
				this->set_led_colors(0, this->light_brightness, 0);
			    break;
			case SLEEP:
				this->set_led_colors(this->light_brightness, this->light_brightness, this->light_brightness);
				break;
		}
	}
	...
	for (k = 0; k< 3; k++) {
		for (i = 0; i< 3; i++) {
			for (j = 0; j < 8; j++) {
				...
				{
					__disable_irq();
					LPC_GPIO0->FIOSET = 1 << 22; //0x00008000;
