...
void Endstops::get_global_configs()
{
	...
	this->cover_endstop_pin2.from_string( THEKERNEL->config->value(cover_endstop2_checksum)->by_default("1.14v" )->as_string())->as_input();
...
void Endstops::on_idle(void *argument)
{
    ...
    if( us_ticker_read() - THEKERNEL->power_24V_time > 300000) {	//lsf add 2026.03.20 wait about 300ms for MKS
...
void Endstops::home_xy()
{
    if(axis_to_home[X_AXIS] && axis_to_home[Y_AXIS]) {
    	if(((MAKERA_Z1 == THEKERNEL->factory_set->MachineModel)||(MAKERA_Z1Pro == THEKERNEL->factory_set->MachineModel)) && THEKERNEL->axis_is_on[A_AXIS])
    	{
    		...
    		// now home X only
	        float delta2[3] {homing_axis[X_AXIS].max_travel, 0, 0};
	        if(homing_axis[X_AXIS].home_direction) delta2[X_AXIS]= -delta2[X_AXIS];
	        THEROBOT->delta_move(delta2, homing_axis[X_AXIS].fast_rate, 3);
    	}
    	else {
...
void Endstops::on_get_public_data(void* argument)
{
    ...
    if(pdr->second_element_is(trim_checksum)) {
        ...
        *homing = this->status != NOT_HOMING;
        ...
        data[5] = a && b;		//lsf modify 2026.04.15
        pdr->set_taken();
    } else if (pdr->second_element_is(get_cover_endstop_state_checksum)) {
        bool *cover_state = static_cast<bool *>(pdr->get_data_ptr());
        *cover_state = this->cover_endstop_pin.get() && this->cover_endstop_pin2.get();				//lsf modify 2026.04.15
        pdr->set_taken();
