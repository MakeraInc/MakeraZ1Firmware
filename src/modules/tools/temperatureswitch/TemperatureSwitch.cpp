...
#define playspindlefanvalue_checksum					CHECKSUM("playspindlefanvalue")
...
TemperatureSwitch* TemperatureSwitch::load_config(uint16_t modcs)
{
    ...
    ts->playspindlefanvalue = THEKERNEL->config->value( playspindlefanvalue_checksum )->by_default(30)->as_number();
    THEKERNEL->set_PlaySpindleFanValue(ts->playspindlefanvalue);
...
void TemperatureSwitch::on_second_tick(void *argument)
{
	...
	if (THEKERNEL->get_laser_mode()) {
	    ...
	    if( (this->temperatureswitch_switch_cs == spindlefan_checksum) && ((MAKERA_Z1 == THEKERNEL->factory_set->MachineModel)||(MAKERA_Z1Pro == THEKERNEL->factory_set->MachineModel)) && (THEKERNEL->is_AutoClean() || THEKERNEL->is_PlaySpindleFan()))
	    {
	    	if (THEKERNEL->is_AutoClean())
	    	{
	    		struct pad_switch pad;
			    pad.state = true;
			    pad.value = 100;
	    	...
	    	else if(THEKERNEL->is_PlaySpindleFan()){
		    	float padvalue = 0;
		    	if (current_temp >= this->temperatureswitch_threshold_temp) {
			    	padvalue = temperatureswitch_cooldown_power_init + (current_temp - temperatureswitch_threshold_temp) * temperatureswitch_cooldown_power_step;
					struct pad_switch pad;
				    bool ok = false;
				    ok = PublicData::get_value(switch_checksum, this->temperatureswitch_switch_cs, 0, &pad);
				    if (ok) {
				    	padvalue = (pad.manualvalue > padvalue) ? pad.manualvalue : padvalue; //zqq modify 2026.07.21
				    	if(padvalue != pad.value){ //zqq modify 2026.07.21
				    		pad.state = true;
			    			pad.value = padvalue;
	    ...
	    else
	    {
		    if (current_temp >= this->temperatureswitch_threshold_temp) {
		    	struct pad_switch pad;
		    	ok = PublicData::get_value(switch_checksum, this->temperatureswitch_switch_cs, 0, &pad); //zqq modify 2026.07.21
