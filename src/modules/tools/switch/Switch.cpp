...
void Switch::on_config_reload(void *argument)
{
    ...
    if(!is_input) {
        if(this->output_type == SIGMADELTA) {
            ...
            this->default_on_value = THEKERNEL->config->value(switch_checksum, this->name_checksum, default_on_value_checksum )->by_default(98)->as_number();
            if(this->switch_state) {
...
uint32_t Switch::pinpoll_tick(uint32_t dummy)
{
    ...
    if(this->input_pin_behavior == freq_count_checksum){
    	if(this->measuring){
    		...
    		if(this->interrupt_count>=210){	//2.1s
    			this->switch_value = (float)this->pulsecount / 2.1; //the frequenc of pwm input (Hz)
    			this->measuring = false;
    			this->interrupt_count = 0;
    			this->pulsecount = 0;
