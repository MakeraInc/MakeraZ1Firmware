...
void Laser::on_console_line_received( void *argument )
{
    ...
    if (cmd == "laser") {
        ...
        if (laser_cmd == "on") {
	        ...
	        if (!THEKERNEL->get_elecremove_mode()){	//lsf add 2026.03.19
...
void Laser::on_gcode_received(void *argument)
{
    ...
    if (gcode->has_m) {
    	if (gcode->m == 3 && THEKERNEL->get_laser_mode())
		{
        	...
        	if (gcode->subcode == 4) {		//lsf add 2026.03.19				
				THEKERNEL->set_elecremove_mode(true);
				this->laser_pin->set(true);
				// turn on static electricity removal  mode
				THEKERNEL->streams->printf("turning static electricity removal mode on\r\n");
			}
        } else if (gcode->m == 332) { // change to CNC mode
        	if (gcode->subcode == 4) {		//lsf add 2026.03.19
				THEKERNEL->set_elecremove_mode(false);
				// turn off static electricity removal mode
