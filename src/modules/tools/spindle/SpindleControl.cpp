    ...
    if (gcode->has_m)
    {
        ...
        else if (gcode->m == 3)
        {
        	if(!b_on_offing){
	        	...
	        	if (THEKERNEL->get_blowing_mode()) {
	        		struct pad_switch pad;
				    bool b = false;
	            	PublicData::set_value( switch_checksum, spindlefan_checksum, state_checksum, &b );       	
	            	pad.state = true;
			    	pad.value = THEKERNEL->get_PlaySpindleFanValue();
			    	PublicData::set_value( switch_checksum, spindlefan_checksum, state_value_checksum, &pad );	
			    			
			    	THEKERNEL->set_PlaySpindleFan(true);
	        	}
	        	b_on_offing = false;
	        }
        }
        else if (gcode->m == 5)
        {
        	if(!b_on_offing){
        		b_on_offing = true;
	        	if (!THEKERNEL->get_laser_mode() && spindle_on) 
	        	{
	                THECONVEYOR->wait_for_idle();
	                // M5: spindle off
	                if (spindle_on) {
			    ...
			    THEKERNEL->set_PlaySpindleFan(false);
			    b_on_offing = false;
			}
        }
        else if (gcode->m == 223)
