...
void PWMSpindleControl::on_module_loaded()
{
    ...
    {
        int legacy_delay_s = THEKERNEL->config->value(spindle_checksum, spindle_delay_s_checksum)->by_default(8)->as_number();
        delay_on_s  = THEKERNEL->config->value(spindle_checksum, spindle_delay_on_s_checksum)->by_default(legacy_delay_s)->as_number();
