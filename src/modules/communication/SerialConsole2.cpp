        ...
        while (1) {
           ...
           if ( c == '\n' ) {
        	   ...
        	   if (received[0] == 'V') {
        		   ...
        		   THEKERNEL->probe_addr = ((uint16_t)received[2]<<24) | received[3]<<16 | received[4]<<8 | received[5];
        		   THEKERNEL->streams->printf("WP power: [%1.2fv], addr: [%u]\n", this->wp_voltage, THEKERNEL->probe_addr);
    ...
    if (gcode->has_m) {
    	if (gcode->m == 470) {
    		if (gcode->has_letter('S')) {
        		uint32_t new_addr = gcode->get_value('S');
        		THEKERNEL->streams->printf("Change WP address to: [%u]\n", new_addr);
        		THEKERNEL->write_ProbeAddr_Data_data(new_addr);
        		THEKERNEL->probe_addr = new_addr;
    		}
    	} else if (gcode->m == 471) {
